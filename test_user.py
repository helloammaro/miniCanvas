...
def test_create_a_user(user_manager):
    ...
    user_manager.create_a_user("Alfredo", "password", "student")

    assert len(user_manager.user_list) == 1
    assert user_manager.user_list[0].name == "Alfredo"
...
def test_find_users(user_manager):
    ...
    user_manager.create_a_user("Geneva", "password1", "teacher")
    user_manager.create_a_user("Lee", "password2", "student")
    user_manager.create_a_user("Jose", "password3", "admin")
    ...
    assert users_found[0].name == "Geneva"
    assert users_found[1].name == "Jose"
